...
Read all sources in this folder, then do this change:

* put a new config parameter "reorder" who express in which order buyed players must be.
* the value is the order by role ()"ROLE" column in players.csv file)
* Example: "reorder": "PDCTA"
* Put this parameter on top of config file.
* Report this parameter on top of save files.
* Add a button "Reorder" (use languages configuration) just left of "Roster Max" info
* When administrator push the "Reorder" button players of all teams are reordered by their role and a new save must be created

**No need to execute any git command**
